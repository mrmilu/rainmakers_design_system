...
class RMVerificationCode extends StatefulWidget {
  /// is completed
  ...
  @override
  State<RMVerificationCode> createState() => _RMVerificationCodeState();
}

class _RMVerificationCodeState extends State<RMVerificationCode> {
    ...
    return KeyboardListener(
      ...
      child: TextField(
        ...
        onChanged: (String value) {
          ...
          if (value.isNotEmpty) {
            String valuePrivate = value;
            int indexPrivate = index;

            while (valuePrivate.isNotEmpty && indexPrivate < widget.length) {
              _listControllerText[indexPrivate].value = TextEditingValue(
                text: valuePrivate[0],
                selection: TextSelection.fromPosition(
                  TextPosition(
                    affinity: TextAffinity.downstream,
                    offset: _listControllerText[indexPrivate].text.length,
                  ),
                ),
              );
              _next(indexPrivate++);
              valuePrivate = valuePrivate.substring(1);
            }
